...
if (isset($_GET['id'])) {
  ...
  $cursosProf = $coursescontroller->getCourseByProfesor($profesorId);
} else {
        ...
        <div class="row">
          <div class="col-md-12">
            ...
            <table class="table table-bordered table-striped table-hover" id="coursesTable">
              ...
              <tbody>
                <?php if (isset($cursosProf['data']) && is_array($cursosProf['data']) && count($cursosProf['data'])): ?>
                  <?php foreach ($cursosProf['data'] as $course): ?>
                    <?php
                    // Obtener el nombre de la categoría por ID
                    $category_id = $course['category_id'];
                    $category = $categorycontroller->getCategoryByID($category_id); // Asumo que tienes un método getCategoryByID()
